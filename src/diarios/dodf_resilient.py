...
class ResilientDodfCollector(DodfCollector):
    ...
    def list_pdf_urls(self, day: date) -> list[str]:
        ...
        for daily_url in self._daily_candidates():
            ...
            try:
                # Requisição deliberadamente sem o adaptador global de retries e
                # sem aquecimento da raiz. Um ConnectTimeout já é evidência
                # suficiente para usar o fallback nesta execução.
                ...
                # www e não-www apontam para a mesma infraestrutura. Não repetir
                # outro timeout de conexão no alias.
                break
    ...
    def collect(self, day: date) -> list[Document]:
        ...
        try:
            ...
            # Erros inesperados também evitam repetição nas datas seguintes, mas
            # continuam registrados no status e no log.
            self._open_circuit(exc)
            primary_error = exc
